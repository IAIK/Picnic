...
static int picnic_test_keys(picnic_params_t parameters) {
  ...
  const size_t diff_key   = instance->input_output_size * 8 - instance->lowmc.n;
  const size_t diff_block = instance->input_output_size * 8 - instance->lowmc.n;
...
static int picnic_test_modified_signatures(picnic_params_t parameters) {
  ...
  for (size_t l = 0; l < signature_len; l += rand() % 256) {
