...
static int lowmc_enc(const picnic_params_t param, const uint8_t* key, const uint8_t* plaintext,
  ...
  if (!mzd_local_equal(ctr, ct, 1, lowmc->n)) {
    print_hex(stdout, sk, pp->output_size);
    printf("\n");
    print_hex(stdout, ctr, pp->output_size);
    printf("\n");
    print_hex(stdout, ct, pp->output_size);
    printf("\n");
    ret = 2;
  }
...
static int LowMC_test_vectorL1_4(void) {
  const uint8_t key[16] = {
...
static int LowMC_test_vectorL3_4(void) {
  const uint8_t key[24] = {
...
static int LowMC_test_vectorL5_4(void) {
