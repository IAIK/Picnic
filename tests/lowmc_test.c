...
static int lowmc_enc(const picnic_params_t param, const uint8_t* key, const uint8_t* plaintext,
  ...
  for(unsigned int i = 0; i < pp->output_size; i++) {
    printf("0x%02X, ", a[i]);
  }
  printf("\n");
...
static int LowMC_test_vectorL3_1_new(void) {
  const uint8_t key[24] = {0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  const uint8_t plaintext[24] = {0xAB, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  const uint8_t ciphertext_expected[24] = {0xB9, 0xB8, 0x54, 0x2F, 0x53, 0x0E, 0xF3, 0x38,
                                           0xA3, 0x5A, 0xA9, 0x85, 0x22, 0xB8, 0xB7, 0xF6,
                                           0xAA, 0xB2, 0xC9, 0x3E, 0x2B, 0x1F, 0x45, 0x05,};
...
static int LowMC_test_vectorL1_1_1_new(void) {
  const uint8_t key[16] = {0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  const uint8_t plaintext[16]           = {0xAB, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  const uint8_t ciphertext_expected[16] = {0x59, 0xDC, 0x24, 0xB8, 0xA5, 0x43, 0xF9, 0x6D,
                                           0xE4, 0xCA, 0xC5, 0x4F, 0xDD, 0x1F, 0x8D, 0x3D,};

  return lowmc_enc(Picnic_L1_1_FS, key, plaintext, ciphertext_expected);
}

static int LowMC_test_vectorL3_1_1_new(void) {
  const uint8_t key[24] = {0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  const uint8_t plaintext[24] = {0xAB, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  const uint8_t ciphertext_expected[24] = {0x35, 0x73, 0xFB, 0x26, 0x02, 0x88, 0x54, 0x89,
                                           0x6B, 0x94, 0x89, 0xCC, 0x7C, 0xC3, 0x29, 0x25,
                                           0x2F, 0x7A, 0x4F, 0xAC, 0xF3, 0x3C, 0x9B, 0x8B,};

  return lowmc_enc(Picnic_L3_1_FS, key, plaintext, ciphertext_expected);
}


static int LowMC_test_vectorL5_1_1_new(void) {
  const uint8_t key[32] = {0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  const uint8_t plaintext[32] = {0xAB, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  const uint8_t ciphertext_expected[32] = {0xA9, 0x50, 0x50, 0x60, 0x0A, 0x9E, 0xE5, 0xA2,
                                           0xB8, 0x3B, 0x93, 0x12, 0xDE, 0x1E, 0x50, 0xFC,
                                           0x2E, 0xBD, 0x00, 0xE0, 0x26, 0x8B, 0xCC, 0xCA,
                                           0x70, 0xED, 0xF4, 0xA4, 0x29, 0xF1, 0xDD, 0x84,};
