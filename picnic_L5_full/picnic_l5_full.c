...
#define LOWMC_BLOCK_BITS LOWMC_255_255_4_N
#define LOWMC_BLOCK_SZ LOWMC_BLOCK_SIZE_Picnic_L5_full
#define PRIVATE_KEY_SIZE PICNIC_PRIVATE_KEY_SIZE_Picnic_L5_full
#define PUBLIC_KEY_SIZE PICNIC_PUBLIC_KEY_SIZE_Picnic_L5_full
#define SIGNATURE_SIZE PICNIC_SIGNATURE_SIZE_Picnic_L5_full
#define PARAM Picnic_L5_full
...
size_t PICNIC_CALLING_CONVENTION picnic_l5_full_signature_size(void) {
  return SIGNATURE_SIZE;
}

size_t PICNIC_CALLING_CONVENTION picnic_l5_full_get_private_key_size(void) {
  return PRIVATE_KEY_SIZE;
}

size_t PICNIC_CALLING_CONVENTION picnic_l5_full_get_public_key_size(void) {
  return PUBLIC_KEY_SIZE;
}

const char* PICNIC_CALLING_CONVENTION picnic_l5_full_get_param_name(void) {
...
int PICNIC_CALLING_CONVENTION picnic_l5_full_sk_to_pk(const picnic_l5_full_privatekey_t* sk,
                                                    picnic_l5_full_publickey_t* pk) {
  if (!sk || !pk) {
    return -1;
  }
...
int PICNIC_CALLING_CONVENTION picnic_l5_full_sign(const picnic_l5_full_privatekey_t* sk,
...
int PICNIC_CALLING_CONVENTION picnic_l5_full_verify(const picnic_l5_full_publickey_t* pk,
                                                  const uint8_t* message, size_t message_len,
...
int PICNIC_CALLING_CONVENTION picnic_l5_full_read_public_key(picnic_l5_full_publickey_t* key,
...
int PICNIC_CALLING_CONVENTION picnic_l5_full_read_private_key(picnic_l5_full_privatekey_t* key,
                                                            const uint8_t* buf, size_t buflen) {
  if (!key || !buf || buflen < 1) {
    return -1;
  }
...
void PICNIC_CALLING_CONVENTION picnic_l5_full_clear_private_key(picnic_l5_full_privatekey_t* key) {
