...
#define LOWMC_BLOCK_BITS LOWMC_256_256_38_N
#define LOWMC_BLOCK_SZ LOWMC_BLOCK_SIZE_Picnic_L5_UR
#define PRIVATE_KEY_SIZE PICNIC_PRIVATE_KEY_SIZE_Picnic_L5_UR
#define PUBLIC_KEY_SIZE PICNIC_PUBLIC_KEY_SIZE_Picnic_L5_UR
#define SIGNATURE_SIZE PICNIC_SIGNATURE_SIZE_Picnic_L5_UR
#define PARAM Picnic_L5_UR
...
size_t PICNIC_CALLING_CONVENTION picnic_l5_ur_signature_size(void) {
  return SIGNATURE_SIZE;
}

size_t PICNIC_CALLING_CONVENTION picnic_l5_ur_get_private_key_size(void) {
  return PRIVATE_KEY_SIZE;
}

size_t PICNIC_CALLING_CONVENTION picnic_l5_ur_get_public_key_size(void) {
  return PUBLIC_KEY_SIZE;
}

const char* PICNIC_CALLING_CONVENTION picnic_l5_ur_get_param_name(void) {
  return "picnic_l5_ur";
}

int PICNIC_CALLING_CONVENTION picnic_l5_ur_keygen(picnic_l5_ur_publickey_t* pk,
  ...
  if (picnic_l5_ur_sk_to_pk(sk, pk)) {
    return -1;
  }
...
int PICNIC_CALLING_CONVENTION picnic_l5_ur_sk_to_pk(const picnic_l5_ur_privatekey_t* sk,
                                                    picnic_l5_ur_publickey_t* pk) {
  if (!sk || !pk) {
    return -1;
  }
...
int PICNIC_CALLING_CONVENTION picnic_l5_ur_validate_keypair(const picnic_l5_ur_privatekey_t* sk,
                                                            const picnic_l5_ur_publickey_t* pk) {
  if (!sk || !pk) {
    return -1;
  }
...
int PICNIC_CALLING_CONVENTION picnic_l5_ur_sign(const picnic_l5_ur_privatekey_t* sk,
...
int PICNIC_CALLING_CONVENTION picnic_l5_ur_verify(const picnic_l5_ur_publickey_t* pk,
                                                  const uint8_t* message, size_t message_len,
...
int PICNIC_CALLING_CONVENTION picnic_l5_ur_write_public_key(const picnic_l5_ur_publickey_t* key,
...
int PICNIC_CALLING_CONVENTION picnic_l5_ur_read_public_key(picnic_l5_ur_publickey_t* key,
...
int PICNIC_CALLING_CONVENTION picnic_l5_ur_write_private_key(const picnic_l5_ur_privatekey_t* key,
                                                             uint8_t* buf, size_t buflen) {
  if (!key || !buf) {
    return -1;
  }
...
int PICNIC_CALLING_CONVENTION picnic_l5_ur_read_private_key(picnic_l5_ur_privatekey_t* key,
                                                            const uint8_t* buf, size_t buflen) {
  if (!key || !buf || buflen < 1) {
    return -1;
  }
...
void PICNIC_CALLING_CONVENTION picnic_l5_ur_clear_private_key(picnic_l5_ur_privatekey_t* key) {
