...
mzd_local_t* mzd_mul_v_sse_128(mzd_local_t* c, mzd_local_t const* v, mzd_local_t const* A) {
  ...
  __m128i cval = _mm_setzero_si128();
  for (unsigned int w = 2; w; --w, ++vptr) {
    word idx = *vptr;
    for (unsigned int i = sizeof(word) * 8; i; --i, idx >>= 1, mAptr += 1) {
      const __m128i mask = _mm_set1_epi64x(-(idx & 1));
      mm128_xor_mask_region(&cval, mAptr, mask, 1);
    }
  }
  *mcptr = cval;
...
mzd_local_t* mzd_addmul_v_sse_128(mzd_local_t* c, mzd_local_t const* v, mzd_local_t const* A) {
  ...
  for (unsigned int w = 2; w; --w, ++vptr) {
    ...
    for (unsigned int i = sizeof(word) * 8; i; --i, idx >>= 1, mAptr += 1) {
      const __m128i mask = _mm_set1_epi64x(-(idx & 1));
      mm128_xor_mask_region(mcptr, mAptr, mask, 1);
    }
  }
...
mzd_local_t* mzd_addmul_v_avx_128(mzd_local_t* c, mzd_local_t const* v, mzd_local_t const* A) {
  ...
  for (unsigned int w = 2; w; --w, ++vptr) {
    word idx = *vptr;
    for (unsigned int i = sizeof(word) * 8; i; i -= 4, idx >>= 4, mAptr += 2) {
      const int64_t m1 = -(idx & 1);
      const int64_t m2 = -((idx >> 1) & 1);
      const int64_t m3 = -((idx >> 2) & 1);
      const int64_t m4 = -((idx >> 3) & 1);

      const __m256i mask1 = _mm256_set_epi64x(m2, m2, m1, m1);
      mm256_xor_mask_region(&cval[0], mAptr, mask1, 1);

      const __m256i mask2 = _mm256_set_epi64x(m4, m4, m3, m3);
      mm256_xor_mask_region(&cval[1], mAptr + 1, mask2, 1);
    }
  }
...
mzd_local_t* mzd_addmul_v_avx_192(mzd_local_t* c, mzd_local_t const* v, mzd_local_t const* A) {
  ...
  for (unsigned int w = 3; w; --w, ++vptr) {
    word idx = *vptr;
    for (unsigned int i = sizeof(word) * 8; i; i -= 2, idx >>= 2, mAptr += 2) {
      const __m256i mask1 = _mm256_set1_epi64x(-(idx & 1));
      mm256_xor_mask_region(&cval[0], mAptr, mask1, 1);

      const __m256i mask2 = _mm256_set1_epi64x(-((idx >> 1) & 1));
      mm256_xor_mask_region(&cval[1], mAptr + 1, mask2, 1);
    }
  }
