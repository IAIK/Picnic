...
int crypto_sign_keypair(unsigned char* pk, unsigned char* sk) {
  ...
  int ret = picnic_keygen(Picnic_L3_UR, &ppk, &psk);
  if (ret) {
    return ret;
  }

  ret = picnic_write_public_key(&ppk, pk, PICNIC_PUBLIC_KEY_SIZE(Picnic_L3_UR));
  if (ret < 0) {
    picnic_clear_private_key(&psk);
    return ret;
  }

  ret = picnic_write_private_key(&psk, sk, PICNIC_PRIVATE_KEY_SIZE(Picnic_L3_UR));
  picnic_clear_private_key(&psk);
  if (ret < 0) {
    return ret;
  }
...
int crypto_sign(unsigned char* sm, unsigned long long* smlen, const unsigned char* m,
  ...
  if (sk[0] != Picnic_L3_UR) {
    return -3;
  }

  size_t signature_len = PICNIC_SIGNATURE_SIZE(Picnic_L3_UR);
  uint32_t len         = 0;

  picnic_privatekey_t psk;
  int ret = picnic_read_private_key(&psk, sk, PICNIC_PRIVATE_KEY_SIZE(Picnic_L3_UR));
  if (ret < 0) {
    picnic_clear_private_key(&psk);
    return ret;
  }
...
int crypto_sign_open(unsigned char* m, unsigned long long* mlen, const unsigned char* sm,
  ...
  picnic_publickey_t ppk;
  int ret = picnic_read_public_key(&ppk, pk, PICNIC_PUBLIC_KEY_SIZE(Picnic_L3_UR));
  if (ret < 0) {
    return ret;
  }
