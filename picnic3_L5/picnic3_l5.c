...
#define LOWMC_BLOCK_SZ LOWMC_BLOCK_SIZE_Picnic3_L5
#define PRIVATE_KEY_SIZE PICNIC_PRIVATE_KEY_SIZE_Picnic3_L5
#define PUBLIC_KEY_SIZE PICNIC_PUBLIC_KEY_SIZE_Picnic3_L5
#define SIGNATURE_SIZE PICNIC_SIGNATURE_SIZE_Picnic3_L5
#define PARAM Picnic3_L5
...
size_t PICNIC_CALLING_CONVENTION picnic3_l5_signature_size(void) {
  return SIGNATURE_SIZE;
}

size_t PICNIC_CALLING_CONVENTION picnic3_l5_get_private_key_size(void) {
  return PRIVATE_KEY_SIZE;
}

size_t PICNIC_CALLING_CONVENTION picnic3_l5_get_public_key_size(void) {
  return PUBLIC_KEY_SIZE;
}

const char* PICNIC_CALLING_CONVENTION picnic3_l5_get_param_name(void) {
  return "picnic3_l5";
}

int PICNIC_CALLING_CONVENTION picnic3_l5_keygen(picnic3_l5_publickey_t* pk,
...
int PICNIC_CALLING_CONVENTION picnic3_l5_validate_keypair(const picnic3_l5_privatekey_t* sk,
                                                            const picnic3_l5_publickey_t* pk) {
  if (!sk || !pk) {
    return -1;
  }
...
int PICNIC_CALLING_CONVENTION picnic3_l5_verify(const picnic3_l5_publickey_t* pk,
                                                  const uint8_t* message, size_t message_len,
...
int PICNIC_CALLING_CONVENTION picnic3_l5_write_public_key(const picnic3_l5_publickey_t* key,
...
int PICNIC_CALLING_CONVENTION picnic3_l5_write_private_key(const picnic3_l5_privatekey_t* key,
                                                             uint8_t* buf, size_t buflen) {
  if (!key || !buf) {
    return -1;
  }
...
void PICNIC_CALLING_CONVENTION picnic3_l5_clear_private_key(picnic3_l5_privatekey_t* key) {
